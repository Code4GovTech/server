...
class SupabaseInterface():
    ...
    def add_discord_metrics(self, discord_metrics):
        ...
        for metric in discord_metrics:
            ...
            if len(data.data) > 0:
                data = self.client.table("discord_metrics").update({"mentor_messages": metric["mentor_messages"],
                                                                    "contributor_messages": metric[
                                                                        "contributor_messages"]}).eq("product_name",
                                                                                                     metric[
    ...
    def add_engagement(self, github_id):
        contributor = self.client.table("contributor_engagement").select("*").eq("contributor_github",
                                                                                 github_id).execute()
        comment_count = contributor.data[0]["github_comments"]
        data = self.client.table("contributor_engagement").update({"github_comments": comment_count + 1}).eq(
            "contributor_github", github_id).execute()
        return data
    ...
    def add_github_metrics(self, github_metrics):
        for metric in github_metrics:
            data = data = self.client.table("github_metrics").select("*").eq("product_name",
                                                                             metric["product_name"]).execute()
            if len(data.data) > 0:
                # this is updatating product name which is bad
                data = self.client.table("github_metrics").update(metric).eq("product_name",
                                                                             metric["product_name"]).execute()
            else:
...
class PostgresORM:
        ...
        try:
            ...
            async with self.session() as session:
                stmt = (
                    select(exists()
                           .where(
                        (GithubClassroomData.discord_id.is_(None)) | (GithubClassroomData.discord_id == discord_id))
                ...
                try:
                    stmt = (
                        update(GithubClassroomData).
                            ...
                            starter_code_url=record.get('starter_code_url',
            ...
            async with  self.session() as session:
                ...
                stmt = (
                    select(Issues, orgs_alias, points_alias)
                        ...
                        .join(points_alias, Issues.complexity == points_alias.complexity, isouter=True)
    ...
    async def get_joined_data_with_filters(self, filters=None):
        async with self.session() as session:
            ...
            return [dict(issue=record[0].to_dict(), org=record[1].to_dict(), points=record[2].to_dict()) for record in
                    records]
            ...
            async with self.session() as session:
                ...
                query = (
                    select(Issues, CommunityOrgs, PointSystem, IssueContributors, ContributorsRegistration)
                        ...
                        .outerjoin(ContributorsRegistration,
    ...
    def add_github_user(self, user):
        data = self.client.table("contributors_registration").upsert(user,
                                                                     on_conflict=["github_id", "discord_id"]).execute()
        return data.data
